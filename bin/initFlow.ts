...
export async function initializeFlow(
	...
	const method = (await promptVars({
		...
		.then((answers) => Object.entries(answers).at(0)?.at(1) ?? "wrangler")
		.then((m) =>
	...
	const dbConfigIndex = wranglerConfig.d1_databases.findIndex(
		(db) => db.binding === "AUTH_DB",
	);

	if (dbConfigIndex === -1) {
		error(
			"Database configuration not found in wrangler.json. Please ensure the database was created successfully and try again.",
		);
		exit(1);
		return;
	}
	wranglerConfig.d1_databases[dbConfigIndex].migrations_dir =
		"drizzle/migrations";
